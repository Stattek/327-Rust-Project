...
fn main() {
    ...
    let maxval: u64 = input.trim().parse().unwrap();
    let number = rand::thread_rng().gen_range(1..=maxval);
    let mut guessed:bool = false;
    let mut wrong_guesses:u64 = 0;
    while guessed != true //loop will end if the random number is guessed.
    {
        let mut guess_str = String::new(); // re-initializes the guess_str for each iteration.
        println!("Enter in a guess: ");
        io::stdin().read_line(&mut guess_str).expect("failed to read user input");
        let guess:u64 = guess_str.trim().parse().unwrap(); //trims and parses the user's input into a unsigned integer
        if guess == number //condition if user guessed correctly
        {
        ...
        else if guess > number //condition if user guessed a number larger than the randomly generated number
        {
            println!("You guessed: {} That is too high", guess);
            wrong_guesses += 1;
        }
        else //condition if user guessed a number smaller than the randomly generated number
        {
